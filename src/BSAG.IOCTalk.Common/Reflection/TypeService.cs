...
namespace BSAG.IOCTalk.Common.Reflection
{
    ...
    public static class TypeService
    {
        ...
        private static AsyncAwaitResultCache taskTResultProviderLast;
        ...
        public static Type BuildInterfaceImplementationType(string interfaceTypeFullname)
        {
            throw new NotSupportedException($"This ioctalk version is compiled for code generation. But no interface implementation for {interfaceTypeFullname} was found! Did you miss to call \"TalkCompositionHost.RegisterAutoGeneratedProxyInterfaceMappings()\"?");
        }
#else
        ...
        private static Type CreateImplementation(Type interfaceType, Type parentType)
        {
            ...
            MetadataReference[] references = GetMetadataReferences(null /*todo: collect ref types from diff assembly*/, interfaceType, parentType);
        ...
        {
            StringBuilder sourceCode = BuildProxyImplementationSource(interfaceType, out List<string> referencesTemp);
        ...
        public static Type BuildProxyImplementation(Type interfaceType)
        {
            throw new NotSupportedException($"This ioctalk version is compiled for code generation. But no proxy interface implementation for {interfaceType.FullName} was found! Did you miss to call \"TalkCompositionHost.RegisterAutoGeneratedProxyInterfaceMappings()\"?");
        }
#else
        ...
        public static Type BuildProxyImplementation(Type interfaceType)
        {
            ...
            StringBuilder source = BuildProxyImplementationSource(interfaceType, out referencedAssemblies);

            return ImplementDynamicType(source.ToString(), "IOCTalk.AutoGeneratedProxies." + interfaceType.Name, GetMetadataReferences(referencedAssemblies, typeof(TypeService), interfaceType));
        }

#endif

        public static StringBuilder BuildProxyImplementationSource(Type interfaceType, out List<string> referencedAssemblies)
        {
        ...
        private static List<string> CreateProxyInterfaceMethodSourceCode(StringBuilder mainSource, Type interfaceType, IList<string> referencedAssemblies)
        {
            ...
            foreach (var method in GetMethodsByType(interfaceType))
            {
                ...
                else
                {
                    ...
                    isAsyncAwait = IsAsyncAwaitType(method.ReturnType, out bool containsAsyncResultValue);
                    if (isAsyncAwait)
                    {
        ...
        /// <returns></returns>
        public static bool IsAsyncAwaitType(Type type, out bool containsResultValue)
        {
            bool isAsyncAwait = typeof(IAsyncResult).IsAssignableFrom(type); // || method.ReturnType.Equals(typeof(ValueTask));      // todo: ValueTask not supported in .net Standard 2.0 ?

            if (isAsyncAwait)
            {
                containsResultValue = type.IsGenericType;
            }
            else
        ...
        public static Type GetAsyncAwaitResultType(Type type)
        {
            if (typeof(IAsyncResult).IsAssignableFrom(type))
            {
                if (type.IsGenericType)
                {
                    ...
                    {
                        currentResultProvider = new AsyncAwaitResultCache(type);
                        taskTResultProviderLast = currentResultProvider;
                    }

                    return currentResultProvider.Property.PropertyType;
                }
                else
                {
                    //return typeof(System.Threading.Tasks.VoidTaskResult)
                }
            }
        ...
        private static MetadataReference[] GetMetadataReferences(List<string> referencedAssemblies = null, params Type[] types)
        {
            ...
            List<string> referenceLocations = referencedAssemblies != null ? referencedAssemblies : new List<string>();
        ...
        private class AsyncAwaitResultCache
        {
            public AsyncAwaitResultCache(Type type)
            {
                Type = type;

                Property = type.GetProperty("Result");
                Getter = TypeService.GenerateGetterFunc(Property);
            }
