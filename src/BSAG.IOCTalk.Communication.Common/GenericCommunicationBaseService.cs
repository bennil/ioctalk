...
namespace BSAG.IOCTalk.Communication.Common
{
    ...
    public class GenericCommunicationBaseService : IGenericCommunicationService
    {
        ...
        public void CreateSession(int sessionId, string description, Action forceCloseCallback, object underlyingCommunicationObject)
        {
        ...
        private void CallReceivedMethod(ISession session, IGenericMessage message)
        {
            ...
            try
            {
                ...
                {
                    throw new Exception($"No implementation for the interface \"{message.Target}\" found! The auto generated proxy \"{implementationInstance.GetType().FullName}\" is not valid for remote invoke requests. Container host: {this.containerHost.Name}; Session: {session.SessionId} - {session.Description}");
                }
