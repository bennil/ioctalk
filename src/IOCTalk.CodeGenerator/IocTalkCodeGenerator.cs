...
//#endif
        ...
        private StringBuilder BuildInterfaceImplementationMappingsSource(ImmutableArray<ITypeSymbol> proxyInterfaceTypes, string assemblyName, List<string> proxyClassNames, List<ITypeSymbol> dtoTypes, List<string> dtoImplementationNames, Dictionary<ITypeSymbol, ITypeSymbol>? interfImplementations)
        {
            ...
            sb.AppendLine("public static class AutoGeneratedInterfaceImplementationMapping { ");

            sb.AppendLine("     public static void UseAutoGeneratedInterfaceImplementationMappings(this LocalShareContext ctx)");
            sb.AppendLine("     {");
            ...
            foreach (var member in interfaceType.GetMembers())
            {
                if (member is IMethodSymbol method)
                {
                    ...
                    else
                    {
                        ...
                        isAsyncAwait = method.IsAsync;
                        if (isAsyncAwait)
                        {
                            //isReturnRequired = containsAsyncResultValue;
                            throw new NotImplementedException();
                        }
                        returnType = $"{method.ReturnType.ContainingNamespace}.{method.ReturnType.Name}";
                    ...
                    if (isReturnRequired)
                    {
                        ...
                        //    {
                        //        string genericTypeStr = method.ReturnType.GetGenericArguments().Select(g => g.FullName).ToArray<string>().First();
